...
import axiosInstance, { getApiBaseUrl } from ../../shared/lib/api-client";
...
export const signIn = async (formData: SignInFormData) => {
  try {
    ...
    // Optimized axios config for faster response
    const response = await axiosInstance.post("/api/auth/login", formData, {
      timeout: 15000, // Increased to 15 seconds to allow for bcrypt comparison
      headers: {
